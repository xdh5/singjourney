export function wrapPcmAsWav(pcm: ArrayBuffer, sampleRate: number) {
  return wrapPcmChunksAsWav([new Uint8Array(pcm)], pcm.byteLength, sampleRate)
}

export function wrapPcmChunksAsWav(
...
export function createLowBitrateWav(
...
export function readMonoSamplesFromWav(wav: ArrayBuffer) {
  const view = new DataView(wav)
  if (
    view.byteLength < WAV_HEADER_SIZE ||
    readAscii(view, 0, 4) !== WAV_RIFF ||
    readAscii(view, 8, 4) !== WAV_FORMAT
  ) {
  ...
  while (offset + 8 <= view.byteLength) {
    ...
    if (chunkId === WAV_FORMAT_CHUNK && chunkSize >= 16) {
      if (view.getUint16(chunkDataOffset, true) !== PCM_FORMAT)
        throw new Error('Unsupported WAV codec')
  ...
  if (
    ...
    !dataOffset ||
    !dataSize ||
    (bitDepth !== PCM_8_BIT && bitDepth !== PCM_16_BIT)
  ) {
