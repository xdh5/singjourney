import { requestAuthenticatedJson } from './authentication'
...
export interface PracticeStatisticsView {
  today: { sessions: number, duration: string }
  total: { sessions: number, duration: string }
...
export const EMPTY_PRACTICE_STATISTICS: PracticeStatisticsView = {
  today: { sessions: 0, duration: '00:00' },
  total: { sessions: 0, duration: '00:00' },
  activity: [],
  todayExercises: []
}
...
export async function fetchPracticeStatistics(): Promise<PracticeStatisticsView> {
  ...
  return {
    ...
    todayExercises: response.today_exercises.map(exercise => ({
      ...
      titleKey: exerciseTitleKey(exercise.exercise_key),
...
function exerciseTitleKey(exerciseKey: string) {
  if (exerciseKey === 'connection-mum-octave') return 'practiceStats.exercises.mumOctave'
  return exerciseKey
}
