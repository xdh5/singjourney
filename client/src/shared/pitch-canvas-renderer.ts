...
export function drawPitchGrid(options: PitchCanvasLayerOptions) {
  ...
  const lastMidi = Math.max(options.minimumMidi, Math.floor(viewportMaxMidi - height / rowHeight) - 1)
  ...
  for (let midi = firstMidi; midi >= lastMidi; midi -= 1) {
    ...
    setStroke(options, pitchClass === 'C' ? getOctaveColor(midi) : DEFAULT_GRID_COLOR, 1)
    context.beginPath()
